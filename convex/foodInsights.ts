...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const doc = {
      ...
      verdict: args.payload.verdict ?? "Unknown",
      facts: args.payload.facts ?? [],
      raw_response: args.payload.raw_response ?? "",
    };
    await ctx.db.insert("foodInsights", doc);
...
export const aggregateCount = action({
  ...
  handler: async (ctx, args) => {
    const rows = await ctx.runQuery("foodInsights:listByUser" as any, {
      clerk_user_id: args.clerk_user_id,
      limit: 1000,
    });
    return rows.length;
